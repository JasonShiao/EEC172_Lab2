...
*  SPI.
*/
    //unsigned long ulUserCmd;
    unsigned long ulDummy; // variable to receive response from slave

    GPIOPinWrite(GPIOA2_BASE, PIN_DC, 0); // pull data pin low to write command
    GPIOPinWrite(GPIOA3_BASE, PIN_OC, 0); // pull chip select low

    //ulUserCmd = c;
    MAP_SPIDataPut(GSPI_BASE, c); // put byte

    //Message("\n\r\r Visit writeCommand func \n\r");
    MAP_SPIDataGet(GSPI_BASE,&ulDummy); // receive response from slave
    GPIOPinWrite(GPIOA3_BASE, PIN_OC, PIN_OC); // pull chip select high to complete transmission
...
*/
    ...
    unsigned long ulDummy; // variable to receive message

    GPIOPinWrite(GPIOA2_BASE, PIN_DC, PIN_DC); // pull DC to write Data
    GPIOPinWrite(GPIOA3_BASE, PIN_OC, 0); // pull chip select low to begin transmission

    //ulUserData = c;
    MAP_SPIDataPut(GSPI_BASE, c); // put data onto the slave

    //Message("\n\r\r Visit writeData func \n\r");
    MAP_SPIDataGet(GSPI_BASE,&ulDummy); // Receive response from slave
    GPIOPinWrite(GPIOA3_BASE, PIN_OC, PIN_OC); // finish transmission by pulling chip select high
...
*/
  ...
  GPIOPinWrite(GPIOA3_BASE, 0x80, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA3_BASE, 0x80, 0x80);	// RESET = RESET_HIGH
